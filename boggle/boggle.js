...
function find(board, word) {
  ...
  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < cols; col++) {
      if (board[row][col] === word[0] && dfs(board, row, col, 0, word, new Array(rows).fill().map(() => new Array(cols).fill(false)))) {
        return true;
      }
    }
  }
...
function dfs(board, row, col, index, word, visited) {
  ...
  const result = dfs(board, row + 1, col, index + 1, word, visited) ||
