...
def find(board, word):
    ...
    def dfs(board, word, i, j, k, visited):
        if not (0 <= i < len(board)) or not (0 <= j < len(board[0])) or visited[i][j] or board[i][j] != word[k]:
            return False
        if k == len(word) - 1:
            return True
        visited[i][j] = True
        found = dfs(board, word, i + 1, j, k + 1, visited) or dfs(board, word, i - 1, j, k + 1, visited) or dfs(board, word, i, j + 1, k + 1, visited) or dfs(board, word, i, j - 1, k + 1, visited)
        visited[i][j] = False
        return found

    for i in range(len(board)):
        for j in range(len(board[0])):
            if dfs(board, word, i, j, 0, [[False for _ in range(len(board[0]))] for _ in range(len(board))]):
                return True
    return False
